      ...
      <div className="max-w-4xl mx-auto space-y-8 bg-white/60 backdrop-blur-md border border-emerald-50 rounded-[2.5rem] p-8 md:p-12 shadow-sm">
        ...
        <form onSubmit={addUnite} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="space-y-2">
            ...
            <input required type="text" className="w-full bg-white border border-emerald-100 rounded-2xl py-3 px-4 text-sm font-bold shadow-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" value={newUnite.nom} onChange={(e) => setNewUnite({...newUnite, nom: e.target.value})} />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Unité (ex: Litre)</label>
            <input required type="text" className="w-full bg-white border border-emerald-100 rounded-2xl py-3 px-4 text-sm font-bold shadow-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" value={newUnite.unite_mesure} onChange={(e) => setNewUnite({...newUnite, unite_mesure: e.target.value})} />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Facteur</label>
            <input required type="number" className="w-full bg-white border border-emerald-100 rounded-2xl py-3 px-4 text-sm font-bold shadow-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" value={newUnite.facteur} onChange={(e) => setNewUnite({...newUnite, facteur: e.target.value})} />
          </div>
        ...
        <div className="border-t border-emerald-50 pt-8">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[10px] text-gray-400 uppercase border-b border-emerald-50">
              ...
              <tbody className="divide-y divide-emerald-50">
                {unites.map(u => (
                  <tr key={u.id} className="group hover:bg-emerald-50/30 transition-colors">
                    ...
                    <td className="py-4 text-sm text-gray-600 font-medium">{u.unite_mesure}</td>
                    <td className="py-4 text-sm text-emerald-600 font-black">{u.facteur}</td>
                ...
                {unites.length === 0 && (
                  <tr>
                    <td colSpan="4" className="py-10 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest">Aucune unité standard enregistrée</td>
                  </tr>
                )}
              </tbody>
            </table>
