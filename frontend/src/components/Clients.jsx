...
import { Plus, Edit2, Trash2, Users, Search, Mail, Phone, MapPin, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="col-span-full text-center py-20 text-gray-400">Chargement des clients...</p>
        ) : filteredClients.length > 0 ? (
          filteredClients.map((c) => (
            <div key={c.id} className="bg-white/60 backdrop-blur-md border border-emerald-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all group relative">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest">Client Fidèle</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-gray-600">
                  ...
                  <span className="text-sm truncate">{c.email || 'Pas d\'email'}</span>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
                  <Phone size={16} className="text-emerald-400" />
                  <span className="text-sm">{c.phone || 'Pas de téléphone'}</span>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
                  <MapPin size={16} className="text-emerald-400" />
                  <span className="text-sm truncate">{c.address || 'Pas d\'adresse'}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-emerald-900/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-4">
              <input required placeholder="Nom complet" className="w-full bg-emerald-50/50 border border-emerald-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              <input type="email" placeholder="Email" className="w-full bg-emerald-50/50 border border-emerald-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
              ...
              <textarea placeholder="Adresse..." className="w-full bg-emerald-50/50 border border-emerald-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})}></textarea>
              <button type="submit" disabled={isSubmitting} className="w-full bg-emerald-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-100 mt-4 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : (editingClient ? "Mettre à jour" : "Enregistrer le client")}
              </button>
            </form>
          </div>
        </div>
